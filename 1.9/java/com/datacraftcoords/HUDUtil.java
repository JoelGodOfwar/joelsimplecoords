...
public class HUDUtil
{
	...
	public static boolean IsMouseoveredBlockRightClickable()
	{
        if (mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK)
        {
            ...
            if(HUDUtil.IsBlockRightClickable(block))
            	return true;
        }
        return false;
	}
    ...
    {
    	int x = mc.objectMouseOver.getBlockPos().getX();	//func_178782_a() friendly name is probably getBlockPos()
	...
	public static int PotionEffectType(){
		String poteff = mc.thePlayer.getActivePotionEffects().toString();
		if (!poteff.isEmpty()){
				if(poteff.contains("effect.moveSlowdown")||poteff.contains("effect.digSlowDown")||poteff.contains("effect.confusion")||
						poteff.contains("effect.blindness")||poteff.contains("effect.hunger")||poteff.contains("effect.weakness")||
						poteff.contains("effect.poison")||poteff.contains("effect.wither")||poteff.contains("effect.glowing")||
						poteff.contains("effect.unluck")||poteff.contains("effect.levitation")){
					return 2;
				}
				if(poteff.contains("effect.moveSpeed")||poteff.contains("effect.digSpeed")||poteff.contains("effect.damageBoost")||
						poteff.contains("effect.jump")||poteff.contains("effect.regeneration")||poteff.contains("effect.resistance")||
						poteff.contains("effect.fireResistance")||poteff.contains("effect.waterBreathing")||poteff.contains("effect.invisibility")||
						poteff.contains("effect.nightVision")||poteff.contains("effect.healthBoost")||poteff.contains("effect.absorption")||
						poteff.contains("effect.saturation")||poteff.contains("effect.luck")){
					return 1;
				}
        }else{
        	return 0;
